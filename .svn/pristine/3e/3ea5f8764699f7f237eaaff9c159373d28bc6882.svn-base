...
//        }
    ...
    public BaseResult<Void> sendOneToSingle(@RequestBody PushRequestLogDTO pushRequestLogDTO)
    {
        ...
        Long id = pushRequestDataService.insert(initData(JSONObject.toJSONString(pushRequestLogDTO), PushRequestLogDTO.class, PushTypeEnum.PUSH_TYPE_1.getCode(), pushRequestLogDTO.getCustomerId().toString()));
        // 发送单条
        cashierSenderNew.sendMsgSync(JSON.toJSONBytes(id), null, MessageEventType.MSG_2_SINGLE.getCode(),
                String.valueOf(pushRequestLogDTO.getCustomerId()), 3);
...
//        return true;
    ...
    public BaseResult<Void> sendMultiToMulti(@RequestBody MessageSendDTO messageSendDTO)
    {
        ...
        Long id = pushRequestDataService.insert(initData(JSONObject.toJSONString(messageSendDTO), MessageSendDTO.class, PushTypeEnum.PUSH_TYPE_5.getCode(), null));
        cashierSenderNew.sendMsgSync(JSON.toJSONBytes(id), null, MessageEventType.MSG_2_MULTI_2_MULTI.getCode(),
