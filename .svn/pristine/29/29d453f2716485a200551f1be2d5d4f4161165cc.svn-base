...
public abstract class MessageBaseController
{
    ...
    protected PageDTO initPageInfo(Integer pageSize, Integer currentPage)
    {
        ...
        int currentPageTemp = pageDTO.getCurrentPage() == 0 ? 1 : pageDTO.getCurrentPage();
        int pageSizeTemp = pageDTO.getPageSize();
        int startRow = (currentPageTemp - 1) * pageSizeTemp;
     ...
     * @return
     */
    protected PageDTO initPageInfo(Integer pageSize, Integer currentPage, Integer startRow)
    {
        PageDTO pageDTO = initPageInfo(pageSize, currentPage);
        if (startRow != null && startRow.intValue() != 0) pageDTO.setStartRow(startRow);
        return pageDTO;
    }
...
//        return checkData(result, requestLogDTO.getDataMap()) ? true : false;
    ...
    {
        cashierSenderNew.sendMsgSync(JSON.toJSONBytes(pushRequestLogMultiDTO), null,
